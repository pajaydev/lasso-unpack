...
function parseLassoBundle(fileName) {
    //if (!options) throw new Error("options is empty, Kindly provide input.");
    //if (!options.input) throw new Error("Invalid input.");
    const fileContent = fs.readFileSync(__dirname + '/build1.js', 'utf8');
    ...
    console.log(path.resolve(options.input, 'lasso-stats.json'));
...
console.log(parseLassoBundle(options));
...
function extractLiterals(stats, args) {
    ...
    if (stats.getType() != null && (stats.getType() === "main" || stats.getType() === "remap")) {
        extractLiteralFromBuiltin(stats, args[0]);
    }
}
